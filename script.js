...
const gatewayMarkers = gateways.map((gateway, index) => {
    ...
    marker.on('click', () => {
        const popupContent = `<div><h3>LoRa Gateway #${index + 1}</h3></div><div><p>RECEIVING...OK</p></div><div><p>SENDING...OK</p></div>`;
...
wells.forEach((well, index) => {
    ...
    gateways.forEach((gateway) => {
        const distance = calculateDistance(well.lat, well.lon, gateway.lat, gateway.lon);
